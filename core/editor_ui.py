...
def _normalize_script_names(raw_scripts):
    """Normalize script names into unique names without prefix/extension."""
...
class EditorUI:
    ...
    def _build_property_inputs(self, obj):
        ...
        attached_scripts = _normalize_script_names(getattr(obj, 'scripts', []))
    ...
    def handle_event(self, event, mouse_pos):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if scripts_info:
                if scripts_info.get('add_btn') and scripts_info['add_btn'].check_click(mouse_pos):
                    ...
                    scripts = _normalize_script_names(entered)
                    if scripts:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            # Handle toggles
            for key in ['use_gravity', 'is_kinematic', 'use_anim_state_controller', 'casts_shadows', 'receives_shadows', 'light_casts_shadows']:
    ...
    def refresh_values(self, obj):
        ...
        if 'scripts' in self.prop_inputs:
            self.prop_inputs['scripts']['attached'] = _normalize_script_names(
                getattr(obj, 'scripts', [])
            )
    ...
    def _draw_properties(self, surface, y, bx, bw):
        ...
        if 'scripts' in self.prop_inputs:
            ...
            attached = _normalize_script_names(info.get('attached', []))
            if attached:
