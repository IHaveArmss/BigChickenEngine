...
class Renderer:
    """Handles clearing, light collection, frustum culling, object rendering,
    ...
    def _render_shadow_meshes(self, renderables, shadow_program, light_vp):
        ...
        for obj in renderables:
            ...
            if has_skin and 'u_bone_matrices' in shadow_program:
                bone_data = obj.animator.bone_bytes
                n = obj.animator.num_bones
                pad = 64 - n
                if pad > 0:
                    bone_data += b''.join(glm.mat4(1.0).to_bytes() for _ in range(pad))
                shadow_program['u_bone_matrices'].write(bone_data)
