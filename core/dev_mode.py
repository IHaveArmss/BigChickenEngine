...
class DevMode:
    ...
    def spawn_at(self, ctx, obj_type, position, scene_objects, rebuild_fn, editor_ui, shader_cache=None, texture_loader=None):
        ...
        if obj_type == 'cube':
            ...
            mesh = Cube(ctx, color=glm.vec3(0.49, 0.48, 1.0), shader_cache=shader_cache)
            fmt = 'cube'
        elif obj_type == 'triangle':
            self.tri_counter += 1
            name = f"tri_{self.tri_counter}"
            mesh = Triangle(ctx, color=glm.vec3(1.0, 0.4, 0.2), shader_cache=shader_cache)
            fmt = 'triangle'
        elif obj_type == 'light':
            self.light_counter += 1
            name = f"light_{self.light_counter}"
            mesh = LightOrb(ctx, radius=0.3, color=glm.vec3(1.0, 1.0, 0.9), shader_cache=shader_cache)
            fmt = 'light'
        else:
            return -1
    ...
    def _create_transform_clip(self, obj, name, keyframes, smooth=True):
        ...
        # Add to animator
        obj.animator.animations[name] = new_clip
    ...
    def add_scripts(self, scene_objects, selected_index, raw_scripts, editor_ui):
        ...
        if selected_index < 0 or selected_index >= len(scene_objects):
            return
        obj = scene_objects[selected_index]
        incoming = normalize_script_names(raw_scripts)
        if not incoming:
            return

        current = normalize_script_names(getattr(obj, 'scripts', []))
        merged = list(current)
        for s in incoming:
            if s not in merged:
                merged.append(s)
    ...
    def remove_scripts(self, scene_objects, selected_index, raw_scripts, editor_ui):
        ...
        if selected_index < 0 or selected_index >= len(scene_objects):
            return
        obj = scene_objects[selected_index]
        targets = set(normalize_script_names(raw_scripts))
        if not targets:
            return
